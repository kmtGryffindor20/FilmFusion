...
for i in range(len(movies)):
    ...
    response = session.get(url+f"{movie_id}", headers=headers)
    data = response.json()
    try:
        total_ratings = data['popularity']
    except:
        total_ratings = 0
    print(movie_id)
    models.Movie.objects.filter(movie_api_id=movie_id).update(popularity=total_ratings)
...
# auth_endpoint = "https://kmtGryffindor20.pythonanywhere.com/api/auth/"
# password = "testing@123"

# session = requests.Session()
# auth_res = session.post(auth_endpoint, json={'username':'KMT', 'password':password})
# print(auth_res.json())
...
# res = session.get('https://kmtgryffindor20.pythonanywhere.com/api/movies/', headers=header)
# print(res.json())
